...
void main ()
{
   ...
   fscanf(input_Configurations,"%[^\n]",string);
	...
	{
   	fscanf(input_Configurations,"%d",&conf);
      ...
      fprintf(output, "\n%d\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%d\t%d",conf,P_size,max_gen,p_m,p_c,k_ncs,k_cs,B_size,life_span,seed);
...
void sendtofile(SOLUTION best[])
{
   ...
   strcat(fil,")_");
	...
	fprintf(output,"Number of items:\t%d\n", number_items);
	fprintf(output,"Bin capacity:\t%d\n", bin_capacity);
	fprintf(output,"L2:\t%d\n", L2);
	...
	{  bins[bin] = 0;
		...
		{  if(p == NULL)
         ...
         fprintf(output, "%d\n",weight[item]);
			...
			{
				printf("ERROR the capacity of bin %d was exceeded", bin);
            binError = bin;
				getchar();
            banError = 1;
			}
	...
	for (j=0; j < n_bins; j++)
	{
      ...
      p = best[j].L.first;
		...
		{  if(p == NULL)
			...
			item = p->data;
			p = p->next;
			fprintf(output, "[Item: %d, Weight: %d]\t", item + 1,weight[item]);
		}
